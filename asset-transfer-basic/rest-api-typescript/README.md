...
The REST API is only intended to work with the [basic asset transfer example](https://github.com/hyperledger/fabric-and-gateway-usages/tree/main/asset-transfer-basic).
...
Clone the `fabric-and-gateway-usages` repository and change to the `fabric-and-gateway-usages/asset-transfer-basic/rest-api-typescript` directory before running the following commands

**Note:** these instructions should work with the main branch of `fabric-and-gateway-usages`
...
```shell
TEST_NETWORK_HOME=$HOME/fabric-and-gateway-usages/test-network npm run generateEnv
```
...
It's also possible to use the [published docker image](https://github.com/hyperledger/fabric-and-gateway-usages/pkgs/container/fabric-rest-sample) to run the sample
...
```shell
TEST_NETWORK_HOME=$HOME/fabric-and-gateway-usages/test-network AS_LOCAL_HOST=false npm run generateEnv
```
