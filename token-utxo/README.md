...
You can run the UTXO token transfer scenario using the Fabric test network. Open a command terminal and navigate to the test network directory in your local clone of the `fabric-and-gateway-usages`. We will operate from the `test-network` directory for the remainder of the tutorial.
...
Open a new terminal to represent Org2 and navigate to fabric-and-gateway-usages/test-network. We'll use the Org2 CA to create the Org2 recipient identity. Set the Fabric CA client home to the MSP of the Org2 CA admin:
