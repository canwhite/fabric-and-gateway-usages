...
You can run the ERC-20 token transfer scenario using the Fabric test network. Open a command terminal and navigate to the test network directory in your local clone of the `fabric-and-gateway-usages`. We will operate from the `test-network` directory for the remainder of the tutorial.
...
Open a 3rd terminal to represent the spender in Org1 and navigate to fabric-and-gateway-usages/test-network. Set the environment variables for the Org1 spender user.
