...
Download the Fabric docker images and binaries. The code only works with Fabric CA 1.5.7+, so even if you cloned the fabric-and-gateway-usages repo before, you may have to re-run it to get the latest versions.

From the fabric-and-gateway-usages directory:
...
```bash
export PATH=</your/path/to/>fabric-and-gateway-usages/bin:$PATH
```

Validate that the CA is at 1.5.7 by executing `fabric-ca-client version`.

> Note: you can run this code from anywhere. If you are _not_ running it from the fabric-and-gateway-usages/token-sdk folder, also set the following environment variable:
>
> ```bash
> export TEST_NETWORK_HOME=</your/path/to>/fabric-and-gateway-usages/test-network
> ```
>
...
As a bonus, this sample contains configuration to connect the [blockchain explorer](https://github.com/hyperledger-labs/blockchain-explorer/) with the fabric-and-gateway-usages network. It allows you to inspect the transactions which are committed to the ledger. It shows more of what the Token SDK does under the covers.
